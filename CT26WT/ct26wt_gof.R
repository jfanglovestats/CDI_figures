...
gc_zinb_size_mle = function(train_data, train_sc, niter = 20){
  ...
  for(i in 1:niter){
    ...
    mu_est = foreach(g = seq_len(ng), .combine = 'c') %dopar% {
      zinb_size_mle_fix_rpi(data[g,], train_sc, iter_r = r_est, iter_pi = pi_est)
    }
...
gof_data = as.matrix(pre_cell[qc2(pre_cell),])
...
saveRDS(gs_nb_pval_df, "wt1_gs_nb_pval_df.rds")
...
saveRDS(gs_zinb_pval_df, "wt1_gs_zinb_pval_df.rds")
...
saveRDS(gc_nb_pval_df, "wt1_gc_nb_pval_df.rds")


# ------------------ gene-common ZINB -------------------------------------- #
gc_zinb_para = gc_zinb_size_mle(train_set, train_sc_vec)
saveRDS(gc_zinb_para, "wt1_zinb_para.rds")
cat("mu= ", gc_zinb_para$mu, "r= ", gc_zinb_para$r, "pi = ", gc_zinb_para$pi, "\n")
...
saveRDS(gc_zinb_pval_df, "wt1_gc_zinb_pval_df.rds")
...
pdf("wt1_pval_dist.pdf")
ggplot(data = pval_df, aes(group = as.factor(Model))) +
  labs(x = "P-value") + theme_bw() + ylim(0,1) +
...
saveRDS(discoveries, "wt1_discoveries.rds")
