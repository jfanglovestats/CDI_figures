...
# ncl_vec = seq(5,100, by = 5)
# method_name = c("Seurat")
lab_df = data.frame(data.frame(tmp = rep(NA, ncol(X_sub))))
...
time_df = data.frame(k = ncl_vec, time = NA)
for(i in seq_len(length(ncl_vec))){
  ...
  aicbic_df = calculate_CDI(
    sub_gcmat = X_sub[1:2,],
    Seurat_obj = NULL,
    cand_lab_df = as.vector(lab_df[,i]),
    ...
    ncore = 1)
  End = Sys.time()  
  time_df[i, "time"] = as.double(difftime(End, Start, units = "secs"), units = "secs")
}




saveRDS(time_df, "lung_all_lab_time.rds")
