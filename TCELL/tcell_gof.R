...
select_cinfo = readRDS("Tcell_5type_filtered_labels.rds")
qc2_mat = readRDS("Tcell_5type_filtered.rds")
lsc123.para = list(K = 5,
                   ncell = nrow(select_cinfo),
                   nfeature = 500,
                   ncore = 40,
                   ngene = nrow(select_gcmat),
                   Celltype = select_cinfo$Group)

para.set = lsc123.para
set.indx = "tcell"
...
stopCluster(cl)
saveRDS(ctc_nb_pval_vec, paste0(set.indx, "ctc_nb_pval_vec.rds"))
...
stopCluster(cl)
saveRDS(ctc_zinb_pval_vec, paste0(set.indx, "ctc_zinb_pval_vec.rds"))
...
stopCluster(cl)
saveRDS(cts_nb_pval_vec, paste0(set.indx, "cts_nb_pval_vec.rds"))
...
stopCluster(cl)
saveRDS(cts_zinb_pval_vec, paste0(set.indx, "cts_zinb_pval_vec.rds"))




ctc_nb_pval_df = readRDS(paste0(set.indx,"ctc_nb_pval_vec.rds"))
ctc_zinb_pval_df = readRDS(paste0(set.indx,"ctc_zinb_pval_vec.rds"))
cts_nb_pval_df = readRDS(paste0(set.indx,"cts_nb_pval_vec.rds"))
cts_zinb_pval_df = readRDS(paste0(set.indx,"cts_zinb_pval_vec.rds"))


pval_range = c(0.005, 0.01, 0.05, 0.1)
model_list = list(ctc_nb_pval_df, ctc_zinb_pval_df, 
                  cts_nb_pval_df, cts_zinb_pval_df)
pval_df = data.frame(
