...
cellsample = list()
ct_names = unique(para.set$Celltype)
for(i in 1:para.set$K){
  cellsample[[i]] <- assign(ct_names[i], X[, para.set$Celltype == ct_names[i]])
}
names(cellsample) = c("regulatory Trm cells", "CD8 Trm cells", "CD8 Tcm, IL17RA+ & CD28+", 
                      "Active EM-like Treg", "Classical CD4 Tem")
ns = length(names(cellsample))
...
train_indx_list = lapply(cellsample, train_indx)
train_list = list(); test_list = list()

for(i in 1:ns){
  train_list[[i]] = cellsample[[i]][,train_indx_list[[i]]]
  test_list[[i]] = cellsample[[i]][,-train_indx_list[[i]]]
}
...
cell_type = NULL
for(i in 1:para.set$K){
  cell_type = c(cell_type, rep(ct_names[i], ng))
}
...
# cell-type-common NB
est_zp_ctc_nb = rep(gs_nb_size_zp(train_mat), para.set$K)
# cell-type-common ZINB
est_zp_ctc_zinb = rep(gs_zinb_size_zp(train_mat), para.set$K)
