...
aicbic_df = readRDS("~/simulation/splat2/splat5cl3_aicbic_df.rds")
cdi_df = aicbic_df %>% 
	mutate(AIC = AIC/(1e+06), BIC = BIC/(1e+06)) %>%
	rename("Cluster_method" =  "method", "N_cluster" = "k", 
		"CDI_AIC" = "AIC" , "CDI_BIC" = "BIC")
saveRDS(cdi_df, "~/simulation/splat2/sd4_cdi_df.rds")


X.sub = X[seu_sig_gname,]
...
maintype_return = CalculateCDI(sub_gcmat = X.sub, 
	                             candidate_label_dataframe = as.vector(para.set$clust.label.real), 
	                             batch_label = NULL, 
...
saveRDS(maintype_return, "sd4_benchmark_return.rds")
benchmark_return = readRDS("sd4_benchmark_return.rds")
...
pdf("sd4_bic_lineplot.pdf", width = 3, height = 3.5)
...
pdf("sd4_bicopt_contingency.pdf", width = 2.5, height = 2.5)
